...
7. ${lease.renewalOption ? 'Lease renewable with mutual agreement' : 'No automatic renewal option'}
...
export const downloadLeasePDF = (lease: LeaseData) => {
  ...
  a.href = url
  a.download = `lease-${lease.id}-${lease.property.name.replace(/\s+/g, '-')}.pdf`
...
export const downloadAllLeasesPDF = (leases: LeaseData[]) => {
  ...
  const allContent = leases.map(lease => generateLeasePDF(lease)).join('\n\n---NEW LEASE---\n\n')
