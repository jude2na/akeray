...
import { FileText, Download, Calendar, TrendingUp, Clock, ArrowLeft, AlertTriangle } from "lucide-react"
...
const expiryTimeline = [
  ...
  { period: "Next 6 months", count: 2, leases: ["Tigist Haile - Bole Apartments", "Dawit Mekonnen - Kazanchis Complex"] }
]
...
export default function LeaseReportPage() {
  ...
  const filteredLeases = leaseData.filter(lease => {
    const matchesProperty = propertyFilter === "all" || lease.property.toLowerCase().includes(propertyFilter)
        ...
        <div className="animate-in fade-in duration-1000">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-lg text-gray-600">Comprehensive analysis of lease agreements and renewals</p>
              <p className="text-sm text-gray-500">Monitor lease expiry dates, renewal rates, and tenant retention</p>
            </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-3">
                    <p className="text-sm font-semibold text-gray-600">Total Leases</p>
                    <p className="text-3xl font-bold text-gray-900">{leaseStats.total}</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-3">
                    ...
                    <p className="text-3xl font-bold text-gray-900">{leaseStats.active}</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-3">
                    ...
                    <p className="text-3xl font-bold text-gray-900">{leaseStats.expiring}</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-3">
                    ...
                    <p className="text-3xl font-bold text-gray-900">{(leaseStats.totalRevenue / 1000).toFixed(0)}K ETB</p>
                  </div>
          ...
          <div className="animate-in fade-in slide-in-from-left-4 duration-700 delay-1050" style={{ animationFillMode: "forwards" }}>
            <Card className="shadow-xl border-0 bg-white/90 backdrop-blur-sm">
              <CardHeader>
                ...
                <CardDescription>Upcoming lease expirations and renewal opportunities</CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    <div key={index} className="space-y-3">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{period.count} lease(s) expiring</p>
                        </div>
                        <div className="text-right">
                          <Badge className={period.count > 0 ? "bg-yellow-100 text-yellow-800" : "bg-emerald-100 text-emerald-800"}>
                            {period.count} leases
                          </Badge>
                        </div>
                      </div>
                      {period.leases.length > 0 && (
                        <div className="ml-4 space-y-1">
                          {period.leases.map((lease, i) => (
                            <p key={i} className="text-xs text-gray-600">• {lease}</p>
                          ))}
                        </div>
                      )}
                    </div>
          ...
          <div className="animate-in fade-in slide-in-from-right-4 duration-700 delay-1200" style={{ animationFillMode: "forwards" }}>
            <Card className="shadow-xl border-0 bg-white/90 backdrop-blur-sm">
              <CardHeader>
                ...
                <CardDescription>Historical lease renewal performance</CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    <div key={index} className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                          <span className="text-sm text-gray-600">{item.renewed} renewed, {item.expired} expired</span>
                          <Badge className={item.rate >= 80 ? "bg-emerald-100 text-emerald-800" : item.rate >= 60 ? "bg-yellow-100 text-yellow-800" : "bg-red-100 text-red-800"}>
                            {item.rate}%
                          </Badge>
                        </div>
                      </div>
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 delay-1350" style={{ animationFillMode: "forwards" }}>
          <Card className="shadow-xl border-0 bg-white/90 backdrop-blur-sm">
            <CardHeader>
              ...
              <CardDescription>Complete lease records and expiry tracking</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                    ...
                    <TableRow key={lease.id} className="hover:bg-gray-50">
                      ...
                      <TableCell>
                        <div className="text-sm">
                          ...
                          <p className="text-gray-500">to {new Date(lease.endDate).toLocaleDateString()}</p>
                        </div>
                      </TableCell>
                      <TableCell>
                        <span className="font-semibold text-emerald-600">{lease.monthlyRent.toLocaleString()} ETB</span>
                      </TableCell>
                      <TableCell>
                        <span className={`font-medium ${getDaysRemainingColor(lease.daysRemaining)}`}>
