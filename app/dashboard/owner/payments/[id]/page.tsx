...
import { CreditCard, Download, Calendar, User, Building, DollarSign, ArrowLeft, Receipt, CheckCircle } from "lucide-react"
        ...
        <div className="animate-in fade-in duration-1000">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-lg text-gray-600">Complete payment information and receipt</p>
              <p className="text-sm text-gray-500">Payment ID: {paymentData.id} • {paymentData.payment.month}</p>
            </div>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="space-y-2">
                  ...
                  <p className="text-3xl font-bold text-emerald-600">{paymentData.payment.amount.toLocaleString()} ETB</p>
                  <p className="text-sm text-gray-600">For {paymentData.payment.month}</p>
                </div>
                <div className="space-y-2">
                  <p className="text-sm font-medium text-gray-500">Payment Date</p>
                  <p className="text-lg font-semibold text-gray-900">{new Date(paymentData.payment.paidDate).toLocaleDateString()}</p>
                  <p className="text-sm text-gray-600">Paid on time</p>
                </div>
                <div className="space-y-2">
                  <p className="text-sm font-medium text-gray-500">Payment Method</p>
                  <p className="text-lg font-semibold text-gray-900">{paymentData.payment.method}</p>
                  <p className="text-sm text-gray-600">Ref: {paymentData.payment.referenceNumber}</p>
                </div>
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-gray-600">Receipt: {paymentData.payment.receiptNumber}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="animate-in fade-in slide-in-from-left-4 duration-700 delay-600" style={{ animationFillMode: "forwards" }}>
            <Card className="shadow-xl border-0 bg-white/90 backdrop-blur-sm">
              <CardHeader>
                ...
                <CardDescription>Details about the tenant who made this payment</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-6">
                  <div className="flex items-center space-x-4 p-4 rounded-2xl bg-gradient-to-r from-blue-50 to-emerald-50 border border-blue-100">
                    <Avatar className="h-16 w-16 ring-4 ring-blue-200">
                      ...
                      <AvatarFallback className="bg-gradient-to-br from-blue-500 to-emerald-500 text-white font-bold text-lg">
                        {paymentData.tenant.name.split(" ").map(n => n[0]).join("")}
                      </AvatarFallback>
                    </Avatar>
                    <div className="flex-1">
                      <p className="font-semibold text-lg text-gray-900">{paymentData.tenant.name}</p>
                      <p className="text-sm text-blue-600 font-medium">Tenant</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center space-x-3 p-3 rounded-xl hover:bg-gray-50 transition-colors">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-900">{paymentData.tenant.phone}</p>
                    ...
                    <div className="flex items-center space-x-3 p-3 rounded-xl hover:bg-gray-50 transition-colors">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-900">{paymentData.tenant.email}</p>
            ...
            <div className="animate-in fade-in slide-in-from-right-4 duration-700 delay-800" style={{ animationFillMode: "forwards" }}>
              <Card className="shadow-xl border-0 bg-white/90 backdrop-blur-sm">
                <CardHeader>
                  ...
                  <CardDescription>Details about the property for this payment</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="p-4 rounded-2xl bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-100">
                    <h3 className="font-semibold text-lg text-gray-900 mb-2">{paymentData.property.name}</h3>
                    <p className="text-purple-600 font-medium text-lg">Unit {paymentData.property.unit}</p>
                    <p className="text-sm text-gray-600 mt-2">{paymentData.property.address}</p>
                  </div>
                </CardContent>
              </Card>
            </div>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center p-3 rounded-xl bg-gray-50">
                      ...
                      <span className="font-semibold">{paymentData.breakdown.baseRent.toLocaleString()} ETB</span>
                    </div>
                    ...
                    {paymentData.breakdown.lateFee > 0 && (
                      <div className="flex justify-between items-center p-3 rounded-xl bg-red-50">
                        ...
                        <span className="font-semibold text-red-600">+{paymentData.breakdown.lateFee.toLocaleString()} ETB</span>
                      </div>
                    )}
                    
                    {paymentData.breakdown.discount > 0 && (
                      <div className="flex justify-between items-center p-3 rounded-xl bg-emerald-50">
                        <span className="text-sm font-medium text-emerald-700">Discount:</span>
                        <span className="font-semibold text-emerald-600">-{paymentData.breakdown.discount.toLocaleString()} ETB</span>
                      </div>
                    )}
                    ...
                    <div className="flex justify-between items-center p-4 rounded-xl bg-gradient-to-r from-emerald-50 to-blue-50 border border-emerald-200">
                      ...
                      <span className="text-2xl font-bold text-emerald-600">{paymentData.breakdown.total.toLocaleString()} ETB</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 delay-1200" style={{ animationFillMode: "forwards" }}>
            <Card className="shadow-xl border-0 bg-white/90 backdrop-blur-sm">
              <CardHeader>
                ...
                <CardDescription>Additional information about this payment</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="p-4 rounded-xl bg-orange-50 border border-orange-200">
                  <p className="text-gray-700 leading-relaxed">{paymentData.payment.notes}</p>
                </div>
              </CardContent>
            </Card>
          </div>
