...
import { FileText, Download, Calendar, User, Building, DollarSign, ArrowLeft, Edit, Phone, Mail } from "lucide-react"
...
const leaseData = {
  ...
  payments: [
    { month: "January 2024", amount: 25000, status: "paid", date: "2024-01-01", method: "Bank Transfer" },
    { month: "February 2024", amount: 25000, status: "paid", date: "2024-02-01", method: "Bank Transfer" },
    { month: "March 2024", amount: 25000, status: "paid", date: "2024-03-01", method: "Cash" },
    { month: "April 2024", amount: 25000, status: "paid", date: "2024-04-01", method: "Bank Transfer" }
  ],
  documents: [
    { name: "Lease Agreement.pdf", type: "lease", date: "2024-01-01", size: "2.3 MB" },
    { name: "Tenant ID Copy.pdf", type: "identification", date: "2024-01-01", size: "1.1 MB" },
    { name: "Payment Receipt Jan.pdf", type: "receipt", date: "2024-01-01", size: "0.8 MB" }
  ]
}
...
export default function LeaseDetailsPage() {
  ...
  const daysUntilExpiry = Math.ceil((new Date(leaseData.lease.endDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))
    ...
    <DashboardLayout userRole="owner" userName="Mulugeta Assefa" userEmail="mulugeta@akeray.et">
      <div className="space-y-8">
        ...
        <div className="animate-in fade-in duration-1000">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-lg text-gray-600">Comprehensive lease information for {leaseData.tenant.name}</p>
              <p className="text-sm text-gray-500">Lease ID: {leaseData.id} • {leaseData.property.name} - Unit {leaseData.property.unit}</p>
            </div>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="space-y-2">
                  ...
                  <p className="text-lg font-semibold text-gray-900">{new Date(leaseData.lease.startDate).toLocaleDateString()}</p>
                  <p className="text-sm text-gray-600">to {new Date(leaseData.lease.endDate).toLocaleDateString()}</p>
                </div>
                <div className="space-y-2">
                  <p className="text-sm font-medium text-gray-500">Monthly Rent</p>
                  <p className="text-lg font-bold text-emerald-600">{leaseData.lease.monthlyRent.toLocaleString()} ETB</p>
                  <p className="text-sm text-gray-600">Due on {leaseData.lease.paymentDueDay}{leaseData.lease.paymentDueDay === 1 ? 'st' : 'th'} of month</p>
                </div>
                <div className="space-y-2">
                  <p className="text-sm font-medium text-gray-500">Security Deposit</p>
                  <p className="text-lg font-semibold text-gray-900">{leaseData.lease.securityDeposit.toLocaleString()} ETB</p>
                  <p className="text-sm text-gray-600">2 months rent</p>
                </div>
                <div className="space-y-2">
                  <p className="text-sm font-medium text-gray-500">Status</p>
                  <Badge className="bg-emerald-100 text-emerald-800 font-semibold">{leaseData.lease.status}</Badge>
          ...
          <div className="animate-in fade-in slide-in-from-left-4 duration-700 delay-600" style={{ animationFillMode: "forwards" }}>
            <Card className="shadow-xl border-0 bg-white/90 backdrop-blur-sm">
              <CardHeader>
                ...
                <CardDescription>Complete tenant details and contact information</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-6">
                  <div className="flex items-center space-x-4 p-4 rounded-2xl bg-gradient-to-r from-blue-50 to-emerald-50 border border-blue-100">
                    <Avatar className="h-16 w-16 ring-4 ring-blue-200">
                      ...
                      <AvatarFallback className="bg-gradient-to-br from-blue-500 to-emerald-500 text-white font-bold text-lg">
                        {leaseData.tenant.name.split(" ").map(n => n[0]).join("")}
                      </AvatarFallback>
                    </Avatar>
                    <div className="flex-1">
                      <p className="font-semibold text-lg text-gray-900">{leaseData.tenant.name}</p>
                      <p className="text-sm text-gray-600">{leaseData.tenant.occupation}</p>
                      <p className="text-sm text-blue-600 font-medium">Tenant since {new Date(leaseData.lease.startDate).toLocaleDateString()}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center space-x-3 p-3 rounded-xl hover:bg-gray-50 transition-colors">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-900">{leaseData.tenant.phone}</p>
                    ...
                    <div className="flex items-center space-x-3 p-3 rounded-xl hover:bg-gray-50 transition-colors">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-900">{leaseData.tenant.email}</p>
                    ...
                    <div className="flex items-center space-x-3 p-3 rounded-xl hover:bg-gray-50 transition-colors">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-900">{leaseData.tenant.emergencyContact}</p>
              ...
              <CardContent>
                <div className="space-y-6">
                  <div className="p-4 rounded-2xl bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-100">
                    <h3 className="font-semibold text-lg text-gray-900 mb-2">{leaseData.property.name}</h3>
                    <p className="text-purple-600 font-medium">Unit {leaseData.property.unit}</p>
                    <p className="text-sm text-gray-600 mt-1">{leaseData.property.address}</p>
                  </div>
                  
                  <div className="grid grid-cols-2 gap-4">
                    <div className="text-center p-4 rounded-xl bg-emerald-50 border border-emerald-200">
                      <p className="text-2xl font-bold text-emerald-600">{leaseData.property.bedrooms}</p>
                      <p className="text-sm text-emerald-700">Bedrooms</p>
                    </div>
                    <div className="text-center p-4 rounded-xl bg-blue-50 border border-blue-200">
                      <p className="text-2xl font-bold text-blue-600">{leaseData.property.bathrooms}</p>
                      <p className="text-sm text-blue-700">Bathrooms</p>
                    </div>
                  </div>
                  
                  <div className="text-center p-4 rounded-xl bg-gray-50 border border-gray-200">
                    <p className="text-2xl font-bold text-gray-600">{leaseData.property.area}</p>
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 delay-1000" style={{ animationFillMode: "forwards" }}>
          <Card className="shadow-xl border-0 bg-white/90 backdrop-blur-sm">
            <CardHeader>
              ...
              <CardDescription>Complete payment record for this lease</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          ...
          <div className="animate-in fade-in slide-in-from-left-4 duration-700 delay-1200" style={{ animationFillMode: "forwards" }}>
            <Card className="shadow-xl border-0 bg-white/90 backdrop-blur-sm">
              <CardHeader>
                ...
                <CardDescription>Key terms and conditions of this lease</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {leaseTerms.map((term, index) => (
                    <div key={index} className="flex items-start space-x-3 p-3 rounded-xl bg-gradient-to-r from-orange-50 to-yellow-50 border border-orange-100">
          ...
          {/* Documents */}
          <div className="animate-in fade-in slide-in-from-right-4 duration-700 delay-1400" style={{ animationFillMode: "forwards" }}>
            <Card className="shadow-xl border-0 bg-white/90 backdrop-blur-sm">
              <CardHeader>
                <CardTitle className="flex items-center space-x-3 text-xl">
              ...
              <CardContent>
                <div className="space-y-4">
                  {leaseData.documents.map((doc, index) => (
                    <div key={index} className="flex items-center justify-between p-4 rounded-xl hover:bg-gray-50 transition-colors border border-gray-100">
        ...
        {/* Important Dates */}
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 delay-1600" style={{ animationFillMode: "forwards" }}>
          <Card className="shadow-xl border-0 bg-white/90 backdrop-blur-sm">
            <CardHeader>
              <CardTitle className="flex items-center space-x-3 text-xl">
              ...
              <CardDescription>Key dates and deadlines for this lease</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="text-center p-6 rounded-2xl bg-gradient-to-r from-emerald-50 to-blue-50 border border-emerald-100">
                  ...
                  <p className="text-lg font-bold text-emerald-600">{new Date(leaseData.lease.startDate).toLocaleDateString()}</p>
                ...
                <div className="text-center p-6 rounded-2xl bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-100">
                  ...
                  <p className="text-lg font-bold text-yellow-600">{new Date(leaseData.lease.endDate).toLocaleDateString()}</p>
                ...
                <div className="text-center p-6 rounded-2xl bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-100">
                  ...
                  <p className="text-lg font-bold text-purple-600">{leaseData.lease.noticePeriod} days</p>
