...
import { Building, Search, Filter, Eye, Calendar, DollarSign, Wrench, FileText, Phone, Mail } from "lucide-react"
...
export default function MyPropertiesPage() {
  ...
  const totalRent = filteredProperties.reduce((sum, p) => sum + p.monthlyRent, 0)
  const activeLeases = filteredProperties.filter(p => p.status === "active").length
        ...
        <div className="animate-in fade-in duration-1000">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-lg text-gray-600">Manage all your current rental properties</p>
              <p className="text-sm text-gray-500">View lease details, payments, and maintenance for each property</p>
            </div>
          </div>
        </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-3">
                    ...
                    <p className="text-3xl font-bold text-gray-900">{(totalRent / 1000).toFixed(0)}K ETB</p>
                  </div>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 delay-600" style={{ animationFillMode: "forwards" }}>
            <Card className="hover:shadow-xl transition-all duration-500 border-l-4 border-purple-200 group cursor-pointer transform hover:scale-105 bg-white/80 backdrop-blur-sm">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-3">
                    <p className="text-sm font-semibold text-gray-600">Maintenance Requests</p>
                    <p className="text-3xl font-bold text-gray-900">{myProperties.reduce((sum, p) => sum + p.maintenanceRequests, 0)}</p>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {filteredProperties.map((property, index) => (
            <div
              ...
              style={{ animationDelay: `${index * 200 + 900}ms`, animationFillMode: "forwards" }}
            >
              <Card className="hover:shadow-xl transition-all duration-500 group cursor-pointer transform hover:scale-105 bg-white/90 backdrop-blur-sm">
                <div className="relative">
                  ...
                  <Badge className={`absolute top-3 right-3 ${getPaymentStatusColor(property.paymentStatus)}`}>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Progress value={getLeaseProgress(property.leaseStart, property.leaseEnd)} className="h-2" />
                  ...
                  <div className="grid grid-cols-2 gap-4 text-center">
                    <div className="p-3 rounded-xl bg-emerald-50 border border-emerald-200">
                      ...
                      <p className="text-lg font-bold text-emerald-600">{property.monthlyRent.toLocaleString()}</p>
                      <p className="text-xs text-emerald-700">Monthly Rent</p>
                    </div>
                    <div className="p-3 rounded-xl bg-blue-50 border border-blue-200">
                      <Calendar className="h-5 w-5 text-blue-600 mx-auto mb-1" />
                      <p className="text-sm font-medium text-blue-600">{new Date(property.nextPayment).toLocaleDateString()}</p>
                  ...
                  <div className="flex items-center space-x-3 p-3 rounded-xl bg-gray-50 border border-gray-200">
                    ...
                    <div className="flex-1">
                      <p className="font-medium text-sm text-gray-900">{property.landlord.name}</p>
                  ...
                  <Button
                    ...
                    asChild
                  >
                    <Link href={`/dashboard/tenant/my-properties/${property.id}`}>
        ...
        {filteredProperties.length === 0 && (
          <Card className="text-center py-12">
            <CardContent>
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">No rented properties found</h3>
