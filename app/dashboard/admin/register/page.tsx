...
import { Users, Plus, Search, Filter, Eye, Check, X, Phone, Mail, FileText, Calendar, User } from "lucide-react"
...
export default function AdminRegisterPage() {
  ...
  const filteredRegistrations = pendingRegistrations.filter((registration) => {
    ...
    const matchesRole = filterRole === "all" || registration.role === filterRole
  ...
  const handleCreateAdmin = () => {
    if (!newAdminData.fullName || !newAdminData.email || !newAdminData.phone || !newAdminData.password) {
      toast({
        title: "Error",
        ...
        <div className="animate-in fade-in duration-1000">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-lg text-gray-600">Review and approve new user registrations</p>
              <p className="text-sm text-gray-500">Manage pending applications and create admin accounts</p>
            </div>
            <Dialog>
              ...
              <DialogContent className="sm:max-w-md">
                ...
                <div className="space-y-4 py-4">
                  ...
                  <Button onClick={handleCreateAdmin} className="w-full bg-gradient-to-r from-emerald-600 to-blue-600 hover:from-emerald-700 hover:to-blue-700">
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-3">
                    <p className="text-sm font-semibold text-gray-600">Pending Applications</p>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 delay-450" style={{ animationFillMode: "forwards" }}>
            <Card className="hover:shadow-xl transition-all duration-500 border-l-4 border-emerald-200 group cursor-pointer transform hover:scale-105 bg-white/80 backdrop-blur-sm">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-3">
                    <p className="text-sm font-semibold text-gray-600">Owner Applications</p>
                    <p className="text-3xl font-bold text-gray-900">{pendingRegistrations.filter(r => r.role === 'owner').length}</p>
                  </div>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 delay-600" style={{ animationFillMode: "forwards" }}>
            <Card className="hover:shadow-xl transition-all duration-500 border-l-4 border-blue-200 group cursor-pointer transform hover:scale-105 bg-white/80 backdrop-blur-sm">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-3">
                    <p className="text-sm font-semibold text-gray-600">Tenant Applications</p>
                    <p className="text-3xl font-bold text-gray-900">{pendingRegistrations.filter(r => r.role === 'tenant').length}</p>
                  </div>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 delay-750" style={{ animationFillMode: "forwards" }}>
            <Card className="hover:shadow-xl transition-all duration-500 border-l-4 border-purple-200 group cursor-pointer transform hover:scale-105 bg-white/80 backdrop-blur-sm">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-3">
                    <p className="text-sm font-semibold text-gray-600">Today's Applications</p>
            ...
            <div className="flex space-x-2">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  <DropdownMenuItem onClick={() => setFilterRole("owner")}>Property Owner</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setFilterRole("tenant")}>Tenant</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                    ...
                    <TableRow key={registration.id} className="hover:bg-gray-50">
                      <TableCell>
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <p className="font-medium text-gray-900">{registration.fullName}</p>
                            <p className="text-sm text-gray-500">Father: {registration.fatherName}</p>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <span className="font-mono text-sm">{registration.idNumber}</span>
                      </TableCell>
                      <TableCell>
                        <span className="text-sm">{new Date(registration.submittedDate).toLocaleDateString()}</span>
                      </TableCell>
                      <TableCell>
            ...
            <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-mono">{selectedRegistration.idNumber}</p>
                    </div>
                    <div>
                      <Label className="text-sm font-medium text-gray-500">Role</Label>
                      <Badge className={getRoleColor(selectedRegistration.role)}>
                ...
                {selectedRegistration.role === "owner" && (
                  <div>
                    <h3 className="text-lg font-semibold mb-3">Property Owner Information</h3>
                    <div className="grid grid-cols-1 gap-4">
                      <div>
                        <Label className="text-sm font-medium text-gray-500">Location</Label>
                        <p className="font-medium">{selectedRegistration.location}</p>
                      </div>
                      {selectedRegistration.googleMapLink && (
                        <div>
                          <Label className="text-sm font-medium text-gray-500">Google Maps Link</Label>
                          <p className="font-medium text-blue-600 truncate">{selectedRegistration.googleMapLink}</p>
                        </div>
                      )}
                      <div>
                        <Label className="text-sm font-medium text-gray-500">Ownership Proof</Label>
                        <p className="font-medium">{selectedRegistration.ownershipProof}</p>
                      </div>
                    </div>
                  </div>
                )}

                {selectedRegistration.role === "tenant" && selectedRegistration.occupation && (
                  <div>
